...
CREATE TABLE StopsAt (
    ...
    CHECK(("order"=1 AND arrivalTime IS NULL AND departureTime IS NOT NULL))
    ),
...
CREATE OR REPLACE FUNCTION CalendarIncludeTripFun() RETURNS trigger AS $$
  DECLARE r RECORD;
  BEGIN
    SELECT calendar INTO r FROM Trip WHERE calendar=OLD.ID;
    IF NOT found THEN RAISE 'No trip includes this calendar';
    ELSE RETURN OLD;
    END IF;
  END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER CalendarIncludeTrip
  AFTER INSERT OR UPDATE ON Calendar DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW EXECUTE PROCEDURE CalendarIncludeTripFun();

CREATE OR REPLACE FUNCTION StopIncludeStopsAtFun() RETURNS trigger AS $$
  DECLARE r RECORD;
  BEGIN
    SELECT latitude,longitude INTO r FROM StopsAt WHERE latitude=OLD.latitude AND longitude=OLD.longitude;
    IF NOT found THEN RAISE 'No trip includes the current stop';
    ELSE RETURN OLD;
    END IF;
  END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER StopIncludeStopsAt
  AFTER INSERT OR UPDATE ON Stop DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW EXECUTE PROCEDURE StopIncludeStopsAtFun();

CREATE OR REPLACE FUNCTION StopIncludeHasNameFun() RETURNS trigger AS $$
  DECLARE r RECORD;
  BEGIN
    SELECT latitude,longitude INTO r FROM StopsHasName WHERE latitude=OLD.latitude AND longitude=OLD.longitude;
    IF NOT found THEN RAISE 'No translation found for the current stop';
    ELSE RETURN OLD;
    END IF;
  END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER StopIncludeHasName
  AFTER INSERT OR UPDATE ON Stop DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW EXECUTE PROCEDURE StopIncludeStopsAtFun();
...
CREATE OR REPLACE FUNCTION TripIncludeStopsAtFun() RETURNS trigger AS $$
  DECLARE r INT;
  BEGIN
    SELECT COUNT(*) INTO r FROM StopsAt WHERE trip=OLD.code;
